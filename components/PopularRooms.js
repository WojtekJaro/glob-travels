...
function PopularRooms({ rooms }) {
  ...
  return (
    <div className="mt-5">
      ...
      <Button onClick={()=>setShowAll(true)} className="py-2 mt-3 border-primary" variant="outlined">
        ...
        <div className="p-5">
          <div className="flex gap-2 mb-4">
            ...
            <div>
              <span className="text-xs font-bold">Max: </span>
              <span className="text-xs">
                {currentRoom?.max} {currentRoom?.max > 1 ? "dorosłych" : "dorosły"}
              </span>
            </div>
          </div>
      ...
      <RoomDrawer isOpen={showAll} setIsOpen={setShowAll}>
      {rooms.slice(0, 2).map((room) => (
        <Card
